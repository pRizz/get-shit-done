...
1. If directory exists, check for any PLAN.md or SUMMARY.md files inside it
...
1. Check if its `<files>` sections or `consumes` fields reference outputs from the target plan
...
1. Run cleanup — handle both first-call and mid-sequence cases:
   ```bash
