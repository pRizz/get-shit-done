...
1. Ensure the notes directory exists (`.planning/notes/` or `~/.claude/notes/`)
1. Generate slug: first ~4 meaningful words of the note text, lowercase, hyphen-separated (strip articles/prepositions from the start)
1. Generate filename: `{YYYY-MM-DD}-{slug}.md`
   - If a file with that name already exists, append `-2`, `-3`, etc.
1. Write the file with frontmatter and note text (see storage format)
1. Confirm with exactly one line: `Noted ({scope}): {note text}`
...
1. Glob `~/.claude/notes/*.md` (if directory exists) — global notes
1. For each file, read frontmatter to get `date` and `promoted` status
1. Exclude files where `promoted: true` from active counts (but still show them, dimmed)
1. Sort by date, number all active entries sequentially starting at 1
1. If total active entries > 20, show only the last 10 with a note about how many were omitted
...
1. Find entry N from the numbered list
1. If N is invalid or refers to an already-promoted note, tell the user and stop
1. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GSD project. Run `/gsd-new-project` to initialize one."
1. Ensure `.planning/todos/pending/` directory exists
1. Generate todo ID: `{NNN}-{slug}` where NNN is the next sequential number (scan both `.planning/todos/pending/` and `.planning/todos/completed/` for the highest existing number, increment by 1, zero-pad to 3 digits) and slug is the first ~4 meaningful words of the note text
1. Extract the note text from the source file (body after frontmatter)
1. Create `.planning/todos/pending/{id}.md`:
...
1. Confirm: `Promoted note {N} to todo {id}: {note text}`
   </step>
