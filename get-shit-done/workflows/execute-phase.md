...
signals are unreliable (see `<runtime-compatibility>`). Set `COPILOT_SEQUENTIAL=true`
...
1. For each plan (sequentially, ignoring wave grouping):
...
1. After all plans: proceed to verification (same as normal mode).
...
1. **Spawn executor agents:**
   ...
   **Sequential mode** (`USE_WORKTREES` is `false`):
   ...
   Omit `isolation="worktree"` from the Task call. Replace the `<parallel-execution>` block with:
...
1. **Post-wave hook validation (parallel mode only):**
...
8. **Execute checkpoint plans between waves** — see `<checkpoint-handling>`.

1. **Proceed to next wave.**
   </step>
...
1. Agent runs until checkpoint task or auth gate → returns structured state
1. Agent return includes: completed tasks table, current task + blocker, checkpoint type/details, what's awaited
1. **Present to user:**
   ```
   ## Checkpoint: [Type]
   ...
   **Progress:** 2/3 tasks complete
   ...
   ```
1. User responds: "approved"/"done" | issue description | decision selection
1. **Spawn continuation agent (NOT resume)** using continuation-prompt.md template:
   - `{completed_tasks_table}`: From checkpoint return
   - `{resume_task_number}` + `{resume_task_name}`: Current task
   - `{user_response}`: What user provided
   - `{resume_instructions}`: Based on checkpoint type
1. Continuation agent verifies previous commits, continues from resume point
1. Repeat until plan completes or user stops
...
1. If found, copy to global store:
...
1. Read both the chain flag and user preference (chain flag already synced in init step):
   ```bash
   AUTO_CHAIN=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
