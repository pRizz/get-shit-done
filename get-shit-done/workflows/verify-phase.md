...
1. What must TESTS PROVE for those truths to be evidenced?
...
1. Derive **artifacts** (concrete file paths for each truth)
1. Derive **key links** (critical wiring where stubs hide)
1. Document the must-haves before proceeding
...
1. Derive **truths** (3-7 observable behaviors, each testable)
1. Derive **artifacts** (concrete file paths for each truth)
1. Derive **key links** (critical wiring where stubs hide)
...
1. IF the previous step produced ANY human verification items:
   → **human_needed** (even if all truths VERIFIED and score is N/N)

1. IF all checks pass AND no human verification items:
   → **passed**
...
1. **Match criteria:** The gap's concern appears in a later phase's goal text, success criteria text, or the later phase's name clearly suggests it covers this area
1. If a clear match is found → move the gap to a `deferred` list with the matching phase reference and evidence text
1. If no match in any later phase → keep as a real `gap`
...
1. **Generate plan per cluster:** Objective, 2-3 tasks (files/action/verify each), re-verify step. Keep focused: single concern per plan.

1. **Order by dependency:** Fix missing → fix stubs → fix wiring → **fix test evidence** → verify.
   </step>
