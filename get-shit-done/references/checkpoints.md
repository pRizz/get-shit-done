...
1. **Claude sets up the verification environment** - Start dev servers, seed databases, configure env vars
1. **User only does what requires human judgment** - Visual checks, UX evaluation, "does this feel right?"
1. **Secrets come from user, automation comes from Claude** - Ask for API keys, then Claude uses them via CLI
1. **Auto-mode bypasses verification/decision checkpoints** — When `workflow._auto_chain_active` or `workflow.auto_advance` is true in config: human-verify auto-approves, decision auto-selects first option, human-action still stops (auth gates cannot be automated)
   </overview>
...
1. **Display checkpoint clearly** using the format below
1. **Wait for user response** - do not hallucinate completion
1. **Verify if possible** - check files, run tests, whatever is specified
1. **Resume execution** - continue to next task only after confirmation
...
1. Stop current task - don't retry repeatedly
1. Create checkpoint:human-action dynamically
1. Provide exact authentication steps
1. Verify authentication works
1. Retry the original task
1. Continue normally
...
1. **checkpoint:decision** (9%) - Human makes architectural/technology choices
1. **checkpoint:human-action** (1%) - Truly unavoidable manual steps with no API/CLI
