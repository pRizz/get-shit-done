...
function cmdValidateHealth(cwd, options, raw) {
  ...
  const nyquistValidationEnabled = (() => {
    if (!fs.existsSync(configPath)) return false;
    try {
      const configRaw = fs.readFileSync(configPath, 'utf-8');
      const configParsed = JSON.parse(configRaw);
      if (typeof configParsed?.workflow?.nyquist_validation === 'boolean') {
        return configParsed.workflow.nyquist_validation;
      }
      if (typeof configParsed?.nyquist_validation === 'boolean') {
        return configParsed.nyquist_validation;
      }
      return false;
    } catch {
      return false;
