...
[なぜ作ったのか](#%E3%81%AA%E3%81%9C%E4%BD%9C%E3%81%A3%E3%81%9F%E3%81%AE%E3%81%8B) · [仕組み](#%E4%BB%95%E7%B5%84%E3%81%BF) · [コマンド](#%E3%82%B3%E3%83%9E%E3%83%B3%E3%83%89) · [なぜ効果的なのか](#%E3%81%AA%E3%81%9C%E5%8A%B9%E6%9E%9C%E7%9A%84%E3%81%AA%E3%81%AE%E3%81%8B) · [ユーザーガイド](docs/ja-JP/USER-GUIDE.md)
...
1. **インストール先** — グローバル（全プロジェクト）またはローカル（現在のプロジェクトのみ）
...
1. **リサーチ** — 並列エージェントが起動しドメインを調査します（オプションですが推奨）
1. **要件定義** — v1、v2、スコープ外を抽出します
1. **ロードマップ** — 要件に紐づくフェーズを作成します
...
1. **プランナーが読む** — どの決定が確定済みかを把握（「無限スクロールに決定」→ スクロール処理を計画に含める）
...
1. **計画** — XML構造で2〜3個のアトミックなタスクプランを作成します
1. **検証** — プランを要件と照合し、合格するまでループします
...
1. **プランごとにフレッシュなコンテキスト** — 実装に200kトークンをフル活用、蓄積されたゴミはゼロ
1. **タスクごとにコミット** — 各タスクが独自のアトミックコミットを取得
1. **目標に対して検証** — コードベースがフェーズの約束を果たしているか確認
...
1. **1つずつ案内** — 「メールでログインできますか？」はい/いいえ、または何が問題かを説明
1. **障害を自動診断** — デバッグエージェントが起動し根本原因を特定
1. **検証済みの修正プランを作成** — 即座に再実行可能
...
| `/gsd-version`      | インストール済みのバージョン、コミット、コミット日時を表示 |
...
- `/gsd-help` または `/gsd-version` を実行してインストールを確認してください
