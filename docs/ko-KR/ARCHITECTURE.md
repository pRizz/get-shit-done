...
1. **멀티 에이전트 오케스트레이션** — 새로운 컨텍스트 윈도우로 전문화된 에이전트를 생성하는 가벼운 오케스트레이터
1. **명세 주도 개발** — 요구 사항 → 조사 → 계획 → 실행 → 검증 파이프라인
1. **상태 관리** — 세션과 컨텍스트 초기화를 넘나드는 영구적인 프로젝트 메모리
...
대부분의 워크플로우 기능 플래그는 키가 없으면 활성화됩니다. Nyquist 검증은 주요 예외로, `workflow.nyquist_validation`이 없으면 `false`로 처리되며 명시적으로 `true`를 설정했을 때만 활성화됩니다.
...
1. **STATE.md 파일 잠금** — 모든 `writeStateMd()` 호출은 lockfile 기반 상호 배제를 사용합니다 (`STATE.md.lock`, `O_EXCL` 원자적 생성). 이는 두 에이전트가 STATE.md를 읽고 서로 다른 필드를 수정하면 마지막 작성자가 다른 에이전트의 변경사항을 덮어쓰는 읽기-수정-쓰기 경쟁 조건을 방지합니다. 오래된 잠금 감지(10초 타임아웃)와 지터를 포함한 스핀 대기가 포함됩니다.
...
1. **위치 선택** — 전역(`--global`) 또는 로컬(`--local`)
1. **파일 배포** — commands, workflows, references, templates, agents, hooks 복사
1. **런타임 적응** — 런타임별 파일 내용 변환.
   - Claude Code: 그대로 사용
...
1. **경로 정규화** — `~/.claude/` 경로를 런타임별 경로로 교체
1. **설정 통합** — 런타임의 `settings.json`에 훅 등록
1. **패치 백업** — v1.17부터 로컬 수정 파일을 `gsd-local-patches/`에 백업하여 `/gsd-reapply-patches`에 사용
1. **매니페스트 추적** — 깔끔한 제거를 위해 `gsd-file-manifest.json` 작성
1. **제거 모드** — `--uninstall`로 모든 GSD 파일, 훅, 설정 제거
...
1. **훅 이벤트 이름** — Claude는 `PostToolUse`를 사용하고 Gemini는 `AfterTool`을 사용합니다
1. **에이전트 전문** — 각 런타임은 고유한 에이전트 정의 형식을 가집니다
1. **경로 규칙** — 각 런타임은 서로 다른 디렉터리에 설정을 저장합니다
1. **모델 참조** — `inherit` 프로필을 통해 GSD가 런타임의 모델 선택에 위임합니다
