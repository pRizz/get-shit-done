...
- [v1.29 기능](#v129-%EA%B8%B0%EB%8A%A5)
  - [Windsurf 런타임 지원](#56-windsurf-%EB%9F%B0%ED%83%80%EC%9E%84-%EC%A7%80%EC%9B%90)
  - [국제화 문서](#57-%EA%B5%AD%EC%A0%9C%ED%99%94-%EB%AC%B8%EC%84%9C)
- [v1.30 기능](#v130-%EA%B8%B0%EB%8A%A5)
  - [GSD SDK](#58-gsd-sdk)
- [v1.31 기능](#v131-%EA%B8%B0%EB%8A%A5)
  - [스키마 드리프트 감지](#59-%EC%8A%A4%ED%82%A4%EB%A7%88-%EB%93%9C%EB%A6%AC%ED%94%84%ED%8A%B8-%EA%B0%90%EC%A7%80)
  - [보안 시행](#60-%EB%B3%B4%EC%95%88-%EC%8B%9C%ED%96%89)
  - [문서 생성](#61-%EB%AC%B8%EC%84%9C-%EC%83%9D%EC%84%B1)
  - [디스커스 체인 모드](#62-%EB%94%94%EC%8A%A4%EC%BB%A4%EC%8A%A4-%EC%B2%B4%EC%9D%B8-%EB%AA%A8%EB%93%9C)
  - [단일 페이즈 자율 모드](#63-%EB%8B%A8%EC%9D%BC-%ED%8E%98%EC%9D%B4%EC%A6%88-%EC%9E%90%EC%9C%A8-%EB%AA%A8%EB%93%9C)
  - [범위 축소 감지](#64-%EB%B2%94%EC%9C%84-%EC%B6%95%EC%86%8C-%EA%B0%90%EC%A7%80)
  - [주장 출처 태깅](#65-%EC%A3%BC%EC%9E%A5-%EC%B6%9C%EC%B2%98-%ED%83%9C%EA%B9%85)
  - [Worktree 토글](#66-worktree-%ED%86%A0%EA%B8%80)
  - [프로젝트 코드 접두사](#67-%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8-%EC%BD%94%EB%93%9C-%EC%A0%91%EB%91%90%EC%82%AC)
  - [Claude Code 스킬 마이그레이션](#68-claude-code-%EC%8A%A4%ED%82%AC-%EB%A7%88%EC%9D%B4%EA%B7%B8%EB%A0%88%EC%9D%B4%EC%85%98)
- [v1.32 기능](#v132-%EA%B8%B0%EB%8A%A5)
  - [STATE.md 일관성 게이트](#69-statemd-%EC%9D%BC%EA%B4%80%EC%84%B1-%EA%B2%8C%EC%9D%B4%ED%8A%B8)
  - [자율 모드 `--to N` 플래그](#70-%EC%9E%90%EC%9C%A8-%EB%AA%A8%EB%93%9C---to-n-%ED%94%8C%EB%9E%98%EA%B7%B8)
  - [리서치 게이트](#71-%EB%A6%AC%EC%84%9C%EC%B9%98-%EA%B2%8C%EC%9D%B4%ED%8A%B8)
  - [검증자 마일스톤 범위 필터링](#72-%EA%B2%80%EC%A6%9D%EC%9E%90-%EB%A7%88%EC%9D%BC%EC%8A%A4%ED%86%A4-%EB%B2%94%EC%9C%84-%ED%95%84%ED%84%B0%EB%A7%81)
  - [Read-Before-Edit 가드 훅](#73-read-before-edit-%EA%B0%80%EB%93%9C-%ED%9B%85)
  - [컨텍스트 축소](#74-%EC%BB%A8%ED%85%8D%EC%8A%A4%ED%8A%B8-%EC%B6%95%EC%86%8C)
  - [디스커스 페이즈 `--power` 플래그](#75-%EB%94%94%EC%8A%A4%EC%BB%A4%EC%8A%A4-%ED%8E%98%EC%9D%B4%EC%A6%88---power-%ED%94%8C%EB%9E%98%EA%B7%B8)
  - [디버그 `--diagnose` 플래그](#76-%EB%94%94%EB%B2%84%EA%B7%B8---diagnose-%ED%94%8C%EB%9E%98%EA%B7%B8)
  - [페이즈 의존성 분석](#77-%ED%8E%98%EC%9D%B4%EC%A6%88-%EC%9D%98%EC%A1%B4%EC%84%B1-%EB%B6%84%EC%84%9D)
  - [안티패턴 심각도 레벨](#78-%EC%95%88%ED%8B%B0%ED%8C%A8%ED%84%B4-%EC%8B%AC%EA%B0%81%EB%8F%84-%EB%A0%88%EB%B2%A8)
  - [방법론 아티팩트 유형](#79-%EB%B0%A9%EB%B2%95%EB%A1%A0-%EC%95%84%ED%8B%B0%ED%8C%A9%ED%8A%B8-%EC%9C%A0%ED%98%95)
  - [플래너 도달 가능성 검사](#80-%ED%94%8C%EB%9E%98%EB%84%88-%EB%8F%84%EB%8B%AC-%EA%B0%80%EB%8A%A5%EC%84%B1-%EA%B2%80%EC%82%AC)
  - [Playwright-MCP UI 검증](#81-playwright-mcp-ui-%EA%B2%80%EC%A6%9D)
  - [Pause-Work 확장](#82-pause-work-%ED%99%95%EC%9E%A5)
  - [응답 언어 설정](#83-%EC%9D%91%EB%8B%B5-%EC%96%B8%EC%96%B4-%EC%84%A4%EC%A0%95)
  - [수동 업데이트 절차](#84-%EC%88%98%EB%8F%99-%EC%97%85%EB%8D%B0%EC%9D%B4%ED%8A%B8-%EC%A0%88%EC%B0%A8)
  - [신규 런타임 지원 (Trae, Cline, Augment Code)](#85-%EC%8B%A0%EA%B7%9C-%EB%9F%B0%ED%83%80%EC%9E%84-%EC%A7%80%EC%9B%90-trae-cline-augment-code)
...
1. **연구** — 스택, 기능, 아키텍처, 위험 요소를 조사하는 4개의 병렬 연구자 에이전트
1. **종합** — 연구 종합자가 결과를 SUMMARY.md로 통합
1. **요구사항** — 사용자 응답과 연구에서 추출하여 범위별로 분류
1. **로드맵** — 요구사항에 매핑된 페이즈 분류, 세분화 설정으로 페이즈 수 제어
...
1. **Visuals** — 초점, 시각적 계층구조, 아이콘 접근성
1. **Color** — 강조색 사용 규율, 60/30/10 준수
1. **Typography** — 글꼴 크기/굵기 제약 준수
1. **Spacing** — 그리드 정렬, 토큰 일관성
1. **Registry Safety** — 서드파티 컴포넌트 검사 요구사항
...
1. 작업 원자성 — 각 작업이 독립적으로 커밋 가능한지 확인
1. 의존성 순서 — 작업이 올바른 순서로 배열되어 있는지 확인
1. 파일 범위 — 계획 간 과도한 파일 중복이 없는지 확인
1. 검증 명령어 — 각 작업에 테스트 가능한 완료 기준이 있는지 확인
1. 컨텍스트 적합성 — 작업이 단일 컨텍스트 창에 맞는지 확인
1. 갭 감지 — 누락된 구현 단계가 없는지 확인
1. Nyquist 준수 — 작업에 자동화된 검증 명령어가 있는지 확인(활성화된 경우)
...
1. **Typography** — 글꼴 크기/굵기 제약 준수
1. **Spacing** — 그리드 정렬, 토큰 일관성
1. **Experience Design** — 로딩/오류/빈 상태 커버리지
...
1. 컨텍스트 모니터가 지표를 읽고 `additionalContext` 경고를 주입합니다.
...
1. 결정 패턴(신속 vs 신중, 위험 허용도)
1. 디버깅 접근 방식(체계적 vs 직관적, 로그 선호도)
1. UX 선호도(디자인 감각, 접근성 인식)
1. 벤더/기술 선택(프레임워크 선호도, 생태계 숙련도)
1. 불만 요인(워크플로우에서 마찰을 일으키는 요소)
1. 학습 스타일(문서 vs 예시, 깊이 선호도)
1. 설명 깊이(고수준 vs 구현 세부 사항)
...
1. **무결성 확인** — 완료된 페이즈에 예상 산출물 파일이 있는지 확인
1. **보고** — `.planning/forensics/`에 저장된 조사 결과가 담긴 마크다운 보고서 생성
1. **이슈** — 팀 가시성을 위해 조사 결과로 GitHub 이슈 생성 제안
...
1. **종합** — 교차 참조가 있는 단일 탐색 가능한 문서로 산출물 결합
1. **출력** — 팀 온보딩과 이해관계자 검토에 적합한 `MILESTONE-SUMMARY.md` 작성
...
1. **전환** — 이후 GSD 명령어를 위한 활성 워크스트림 컨텍스트 변경
1. **관리** — 워크스트림 나열, 상태 확인, 진행 상황 추적, 완료, 재개
...
1. **표시** — 페이즈 의존성, 충돌, 진행 상황을 보여주는 개요 렌더링
1. **상호작용** — 개별 페이즈를 탐색, 검사, 또는 작업하는 명령어 수락
...
1. **분류** — 가정을 신뢰도 수준별로 분류: Confident, Likely, Unclear
1. **게이트** — 모든 가정이 HIGH 신뢰도라면 질문 완전히 건너뛰기
1. **확인** — 불명확한 가정을 사용자에게 타겟팅된 질문으로 제시
1. **출력** — 기본 논의 모드와 동일한 형식으로 `{phase}-CONTEXT.md` 생성
...
1. **주석** — ROADMAP.md의 해당 페이즈에 `ui_hint` 표시 추가
1. **표시** — UI 중심 페이즈의 다음 단계에 `/gsd-ui-phase` 권장사항 포함
...
1. **프롬프트** — 런타임 선택을 위한 다중 선택 인터페이스 표시
1. **설치** — 단일 세션에서 선택된 모든 런타임에 GSD 구성
...
1. **변환** — GSD 프롬프트를 Windsurf 규칙 형식으로 변환
1. **설치** — Windsurf 구성 디렉토리에 GSD 설정
...
1. **게시** — 번역된 문서를 영어 원본과 함께 접근 가능하게 제공
...
1. **구성** — 프로젝트 경로와 워크플로우 옵션을 프로그래밍 방식으로 설정
1. **실행** — API 호출로 GSD 페이즈(discuss, plan, execute) 실행
...
1. **확인** — 계획에 대응하는 마이그레이션/push 명령이 포함되어 있는지 확인
1. **게이트** — 마이그레이션 없는 스키마 드리프트가 감지되면 실행 차단(실행 시점 게이트)
1. **주입** — 계획 생성 중 마이그레이션 리마인더 추가(계획 시점 주입)
...
1. **감사** — `gsd-security-auditor`를 스폰하여 위협 모델에 대해 검증
1. **게이트** — 발견 사항이 `security_block_on` 심각도 이상이면 페이즈 진행 차단
...
1. **검증** — `gsd-doc-verifier`를 스폰하여 코드베이스에 대한 문서 정확성 검사
1. **출력** — 정확성 주석이 포함된 검증된 문서 생성
...
1. **플랜** — 수집된 컨텍스트로 플랜 페이즈 자동 호출
1. **실행** — 생성된 계획으로 실행 페이즈 자동 호출
...
1. **실행** — 해당 페이즈에 대해 전체 자율 플로우(discuss → plan → execute) 실행
1. **중단** — 다음 페이즈로 진행하지 않고 페이즈 완료 후 중단
...
1. **검사** — 플랜 체커가 모든 페이즈 요구사항이 계획에 포함되어 있는지 확인
1. **복구** — 오케스트레이터가 삭제된 요구사항을 감지하고 계획 루프에 재주입
...
1. **태그** — 각 주장에 출처(파일 경로, 문서, API 응답)를 주석으로 추가
1. **분리** — 직접적 증거가 없는 가정을 별도 섹션에 기록
...
1. **마이그레이션** — 각 GSD 명령어에 대해 `skills/gsd-*/SKILL.md` 파일 작성
1. **정리** — 스킬이 설치되면 레거시 `commands/gsd/` 디렉토리 제거
1. **폴백** — 이전 Claude Code 버전을 위한 Gemini 경로 호환성 유지
...
1. **동기화** — 드리프트가 감지되면 디스크에서 STATE.md 재구성
1. **전환** — 실행 페이즈 준비 상태로 계획 수와 함께 포스트 플래닝 상태 기록
...
1. **실행** — 페이즈 N까지(포함) 각 페이즈에 대해 자율 플로우 실행
1. **중단** — 페이즈 N 완료 후 중단
...
1. **게이트** — 미해결 질문이 발견되면 계획 차단
1. **표시** — 해결이 필요한 구체적인 오픈 질문 표시
...
1. **필터** — 감지된 갭을 후속 마일스톤 페이즈와 교차 참조
1. **분류** — 연기된 항목을 진정한 갭과 별도로 표시
...
1. **절삭** — 과대 아티팩트에 Markdown 인식 절삭 적용
1. **순서화** — KV 캐시 재사용 최적화를 위해 프롬프트 섹션 배치
