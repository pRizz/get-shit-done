...
- [워크플로우 다이어그램](#%EC%9B%8C%ED%81%AC%ED%94%8C%EB%A1%9C%EC%9A%B0-%EB%8B%A4%EC%9D%B4%EC%96%B4%EA%B7%B8%EB%9E%A8)
- [UI 설계 계약](#ui-%EC%84%A4%EA%B3%84-%EA%B3%84%EC%95%BD)
- [백로그 및 스레드](#%EB%B0%B1%EB%A1%9C%EA%B7%B8-%EB%B0%8F-%EC%8A%A4%EB%A0%88%EB%93%9C)
- [워크스트림](#%EC%9B%8C%ED%81%AC%EC%8A%A4%ED%8A%B8%EB%A6%BC)
- [보안](#%EB%B3%B4%EC%95%88)
- [명령어 레퍼런스](#%EB%AA%85%EB%A0%B9%EC%96%B4-%EB%A0%88%ED%8D%BC%EB%9F%B0%EC%8A%A4)
- [설정 레퍼런스](#%EC%84%A4%EC%A0%95-%EB%A0%88%ED%8D%BC%EB%9F%B0%EC%8A%A4)
- [사용 예시](#%EC%82%AC%EC%9A%A9-%EC%98%88%EC%8B%9C)
- [문제 해결](#%EB%AC%B8%EC%A0%9C-%ED%95%B4%EA%B2%B0)
- [복구 빠른 레퍼런스](#%EB%B3%B5%EA%B5%AC-%EB%B9%A0%EB%A5%B8-%EB%A0%88%ED%8D%BC%EB%9F%B0%EC%8A%A4)
...
**활성화:** 검증 아키텍처와 8번째 plan-check 차원이 필요한 페이즈에서는 `/gsd-settings`에서 `workflow.nyquist_validation: true`로 설정하세요.
...
1. 구조화된 가정 목록을 생성합니다 (기술 선택, 패턴, 파일 위치).
1. 가정을 확인, 수정 또는 확장하도록 제시합니다.
1. 확인된 가정으로 CONTEXT.md를 작성합니다.
...
1. 디자인 시스템 상태를 감지합니다 (shadcn components.json, Tailwind 설정, 기존 토큰).
1. shadcn 초기화 게이트 — React/Next.js/Vite 프로젝트에 없으면 초기화를 제안합니다.
1. 아직 답변되지 않은 설계 계약 질문만 묻습니다 (간격, 타이포그래피, 색상, 카피라이팅, 레지스트리 안전).
1. 페이즈 디렉터리에 `{phase}-UI-SPEC.md`를 작성합니다.
1. 6개 차원에 대해 검증합니다 (카피라이팅, 시각, 색상, 타이포그래피, 간격, 레지스트리 안전).
1. BLOCKED인 경우 수정 루프 (최대 2회 반복).
...
1. 시각 — 초점, 시각적 계층, 아이콘 접근성
1. 색상 — 강조 사용 규율, 60/30/10 준수
1. 타이포그래피 — 폰트 크기/굵기 제약 준수
1. 간격 — 그리드 정렬, 토큰 일관성
1. 경험 디자인 — 로딩/오류/빈 상태 커버리지
...
1. 프리셋 문자열을 복사합니다.
1. `npx shadcn init --preset {paste}`를 실행합니다.
1. 프리셋은 전체 디자인 시스템(색상, 테두리 반경, 폰트)을 인코딩합니다.
...
| `workflow.nyquist_validation`        | `true`, `false`           | `false`    | plan-phase 중 검증 아키텍처 조사 및 8번째 plan-check 차원                                                 |
...
- **inherit** -- 모든 에이전트가 현재 세션 모델을 사용합니다. 동적으로 모델을 전환할 때 (예: OpenCode 또는 Kilo `/model`) 또는 예상치 못한 API 비용을 방지하기 위해 비Anthropic 공급자 (OpenRouter, 로컬 모델)와 함께 Claude Code를 사용할 때 적합합니다. 비Claude 런타임 (Codex, OpenCode, Gemini CLI, Kilo)의 경우 설치 프로그램이 자동으로 `resolve_model_ids: "omit"`을 설정합니다 — [비Claude 런타임](#%EB%B9%84claude-%EB%9F%B0%ED%83%80%EC%9E%84-codex-opencode-gemini-cli-kilo-%EC%82%AC%EC%9A%A9)을 참고하세요.
