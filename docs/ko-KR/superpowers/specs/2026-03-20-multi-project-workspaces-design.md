...
1. **입력 수집** — `--name`/`--repos`/`--path`가 제공되지 않으면 대화형으로 질문합니다. 저장소의 경우 cwd의 하위 `.git` 디렉토리를 옵션으로 표시합니다.
1. **유효성 검사** — 대상 경로가 존재하지 않거나 비어 있음. 소스 저장소가 존재하고 git 저장소임.
1. **워크스페이스 디렉토리 생성** — `mkdir -p <path>`
1. **저장소 복사** — 각 저장소에 대해.
   - Worktree: `git worktree add <workspace>/<repo-name> -b workspace/<name>`
   - Clone: `git clone <source> <workspace>/<repo-name>`
1. **WORKSPACE.md 작성** — 소스 경로, 전략, 브랜치가 있는 매니페스트
1. **.planning/ 초기화** — `mkdir -p <workspace>/.planning`
1. **/gsd-new-project 제안** — 새 워크스페이스에서 프로젝트 초기화를 실행할지 사용자에게 질문
1. **커밋** — commit_docs가 활성화된 경우 WORKSPACE.md의 원자적 커밋
1. **완료** — 워크스페이스 경로와 다음 단계 출력
...
1. WORKSPACE.md 생성 — 저장소 표, 전략, 날짜가 있는 올바른 형식
1. 저장소 발견 — cwd 하위 항목에서 `.git` 디렉토리 식별, git 디렉토리가 아닌 디렉토리와 파일 건너뜀
1. 유효성 검사 — 비어 있지 않은 기존 대상 경로 거부, git이 아닌 소스 경로 거부

### 통합 테스트(동일 파일)

5. Worktree 생성 — 워크스페이스 생성, 저장소 디렉토리가 유효한 git worktree인지 확인
1. Clone 생성 — 워크스페이스 생성, 저장소가 독립적인 클론인지 확인
1. 워크스페이스 나열 — 두 개의 워크스페이스 생성, 나열 출력에 둘 다 포함되는지 확인
1. 워크스페이스 제거 — worktree로 워크스페이스 생성, 제거, 정리 확인
1. 부분 실패 — 유효한 저장소 하나 + 유효하지 않은 경로 하나, 유효한 저장소만으로 워크스페이스 생성
