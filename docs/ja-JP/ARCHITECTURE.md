...
- [設計原則](#%E8%A8%AD%E8%A8%88%E5%8E%9F%E5%89%87)
- [コンポーネントアーキテクチャ](#%E3%82%B3%E3%83%B3%E3%83%9D%E3%83%BC%E3%83%8D%E3%83%B3%E3%83%88%E3%82%A2%E3%83%BC%E3%82%AD%E3%83%86%E3%82%AF%E3%83%81%E3%83%A3)
- [エージェントモデル](#%E3%82%A8%E3%83%BC%E3%82%B8%E3%82%A7%E3%83%B3%E3%83%88%E3%83%A2%E3%83%87%E3%83%AB)
- [データフロー](#%E3%83%87%E3%83%BC%E3%82%BF%E3%83%95%E3%83%AD%E3%83%BC)
- [ファイルシステムレイアウト](#%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB%E3%82%B7%E3%82%B9%E3%83%86%E3%83%A0%E3%83%AC%E3%82%A4%E3%82%A2%E3%82%A6%E3%83%88)
- [インストーラーアーキテクチャ](#%E3%82%A4%E3%83%B3%E3%82%B9%E3%83%88%E3%83%BC%E3%83%A9%E3%83%BC%E3%82%A2%E3%83%BC%E3%82%AD%E3%83%86%E3%82%AF%E3%83%81%E3%83%A3)
- [フックシステム](#%E3%83%95%E3%83%83%E3%82%AF%E3%82%B7%E3%82%B9%E3%83%86%E3%83%A0)
- [CLIツールレイヤー](#cli%E3%83%84%E3%83%BC%E3%83%AB%E3%83%AC%E3%82%A4%E3%83%A4%E3%83%BC)
- [ランタイム抽象化](#%E3%83%A9%E3%83%B3%E3%82%BF%E3%82%A4%E3%83%A0%E6%8A%BD%E8%B1%A1%E5%8C%96)
...
1. **マルチエージェントオーケストレーション** — 専門エージェントをフレッシュなコンテキストウィンドウで起動する軽量オーケストレーター
1. **仕様駆動開発** — 要件 → 調査 → 計画 → 実行 → 検証のパイプライン
1. **状態管理** — セッションやコンテキストリセットをまたいだ永続的なプロジェクトメモリ
...
ほとんどのワークフロー機能フラグは未設定なら有効です。Nyquist バリデーションは主要な例外で、`workflow.nyquist_validation` が未設定の場合は `false` になり、明示的に `true` を設定したときだけ有効になります。
...
1. **STATE.md ファイルロック** — すべての `writeStateMd()` 呼び出しはロックファイルベースの相互排他（`STATE.md.lock`、`O_EXCL` によるアトミック作成）を使用します。これにより、2つのエージェントがSTATE.mdを読み取り、異なるフィールドを変更し、最後の書き込みが他方の変更を上書きする読み取り-変更-書き込みの競合状態を防止します。古いロックの検出（10秒タイムアウト）とジッター付きのスピンウェイトを含みます。
...
1. **インストール先の選択** — グローバル（`--global`）またはローカル（`--local`）
1. **ファイルデプロイ** — コマンド、ワークフロー、リファレンス、テンプレート、エージェント、フックをコピー
1. **ランタイム適応** — ランタイムごとにファイル内容を変換：
   - Claude Code: そのまま使用
...
1. **パス正規化** — `~/.claude/` パスをランタイム固有のパスに置換
1. **設定統合** — ランタイムの `settings.json` にフックを登録
1. **パッチバックアップ** — v1.17以降、ローカルで変更されたファイルを `/gsd-reapply-patches` 用に `gsd-local-patches/` へバックアップ
1. **マニフェスト追跡** — クリーンアンインストールのために `gsd-file-manifest.json` を書き込み
1. **アンインストールモード** — `--uninstall` ですべてのGSDファイル、フック、設定を削除
...
1. **フックイベント名** — Claude Codeは `PostToolUse`、Geminiは `AfterTool` を使用
1. **エージェントフロントマター** — 各ランタイムは独自のエージェント定義形式を持つ
1. **パス規約** — 各ランタイムは異なるディレクトリに設定を保存
1. **モデル参照** — `inherit` プロファイルにより、GSDはランタイムのモデル選択に委譲
