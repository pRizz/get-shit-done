...
[我为什么开发这个](#%E6%88%91%E4%B8%BA%E4%BB%80%E4%B9%88%E5%BC%80%E5%8F%91%E8%BF%99%E4%B8%AA) · [工作原理](#%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86) · [命令](#%E5%91%BD%E4%BB%A4) · [为什么有效](#%E4%B8%BA%E4%BB%80%E4%B9%88%E6%9C%89%E6%95%88) · [用户指南](USER-GUIDE.md)
...
1. **研究** —— 生成并行代理调查领域（可选但推荐）
1. **需求** —— 提取哪些是 v1、v2 和范围外
1. **路线图** —— 创建映射到需求的阶段
...
1. **规划者读取它** —— 知道哪些决策已锁定（"无限滚动已决定" → 规划包含滚动处理）
...
1. **规划** —— 创建 2-3 个带有 XML 结构的原子任务计划
1. **验证** —— 根据需求检查计划，循环直到通过
...
1. **每个计划全新上下文** —— 200k token 纯粹用于实现，零累积垃圾
1. **每个任务提交** —— 每个任务都有自己的原子提交
1. **根据目标验证** —— 检查代码库是否交付了阶段承诺的内容
...
1. **逐个引导你** —— "你能用邮箱登录吗？" 是/否，或描述有什么问题
1. **自动诊断失败** —— 生成调试代理找根本原因
1. **创建已验证的修复计划** —— 准备立即重新执行
...
GSD 在 `.planning/config.json` 中存储项目设置。在 `/gsd-new-project` 期间配置或稍后用 `/gsd-settings` 更新。完整配置模式、工作流开关、git 分支选项和每个代理的模型分解，请参阅[用户指南](USER-GUIDE.md#%E9%85%8D%E7%BD%AE%E5%8F%82%E8%80%83)。
