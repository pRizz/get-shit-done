...
1. 按项目约定创建测试文件
1. 编写描述预期行为的测试（来自 `<behavior>` 元素）
1. 运行测试 - 必须**失败**
1. 如果测试通过：功能已存在或测试有误。调查。
1. 提交：`test({phase}-{plan}): add failing test for [feature]`
...
1. 不耍小聪明，不优化 - 只让它工作
1. 运行测试 - 必须**通过**
1. 提交：`feat({phase}-{plan}): implement [feature]`

**REFACTOR（如需要）：**

1. 如果存在明显的改进，清理实现
1. 运行测试 - 必须**仍然通过**
1. 仅在做出更改时提交：`refactor({phase}-{plan}): clean up [feature]`
