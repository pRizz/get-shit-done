...
[我为什么做这个](#%E6%88%91%E4%B8%BA%E4%BB%80%E4%B9%88%E5%81%9A%E8%BF%99%E4%B8%AA) · [它是怎么工作的](#%E5%AE%83%E6%98%AF%E6%80%8E%E4%B9%88%E5%B7%A5%E4%BD%9C%E7%9A%84) · [命令](#%E5%91%BD%E4%BB%A4) · [为什么它有效](#%E4%B8%BA%E4%BB%80%E4%B9%88%E5%AE%83%E6%9C%89%E6%95%88) · [用户指南](docs/USER-GUIDE.md)
...
1. **研究**：并行拉起代理调研领域知识（可选，但强烈建议）
1. **需求梳理**：提取哪些属于 v1、v2，哪些不在范围内
1. **路线图**：创建与需求映射的阶段规划
...
1. **规划代理会读取它**：知道哪些决策已经锁定（例如“已决定使用无限滚动” → 计划里就会包含滚动处理）
...
1. **制定计划**：创建 2-3 份原子化任务计划，使用 XML 结构
1. **验证**：将计划与需求对照检查，直到通过为止
...
1. **每个计划使用新上下文**：20 万 token 纯用于实现，零历史垃圾
1. **每个任务单独提交**：每项任务都有自己的原子提交
1. **对照目标验证**：检查代码库是否真的交付了该阶段承诺的内容
...
1. **逐项带你验证**：“能否用邮箱登录？” 可以 / 不可以，或者描述哪里不对
1. **自动诊断失败**：拉起 debug 代理定位根因
1. **创建验证过的修复计划**：可立刻重新执行
...
- 运行 `/gsd-help` 或 `/gsd-version` 确认安装成功
