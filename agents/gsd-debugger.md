...
1. **Question your design decisions** - Your implementation decisions are hypotheses, not facts
1. **Admit your mental model might be wrong** - The code's behavior is truth; your model is a guess
1. **Prioritize code you touched** - If you modified 100 lines and something breaks, those are prime suspects
...
1. **You can't explain the current behavior** - Don't add changes on top of confusion
1. **You're debugging the debugger** - Something fundamental is wrong
1. **The fix works but you don't know why** - This isn't fixed, this is luck

**Restart protocol:**

1. Close all files and terminals
1. Write down what you know for certain
1. Write down what you've ruled out
1. List new hypotheses (different from before)
1. Begin again from Phase 1: Evidence Gathering
...
1. **Ask "What could cause this?"** - List every possible cause (don't judge yet)
1. **Make each specific:** Not "state is wrong" but "state is updated twice because handleClick is called twice"
1. **Identify evidence:** What would support/refute each hypothesis?
...
1. **Test setup:** What do I need to do?
1. **Measurement:** What exactly am I measuring?
1. **Success criteria:** What confirms H? What refutes H?
1. **Run:** Execute the test
1. **Observe:** Record what actually happened
1. **Conclude:** Does this support or refute H?
...
1. **Reproduce reliably?** Either always reproduces, or you understand trigger conditions
1. **Have evidence, not just theory?** You've observed directly, not guessing
1. **Ruled out alternatives?** Evidence contradicts other hypotheses
...
1. **Extract the learning** - What did this rule out? What new information?
1. **Revise understanding** - Update mental model
1. **Form new hypotheses** - Based on what you now know
1. **Don't get attached** - Being wrong quickly is better than being wrong slowly
...
1. Add logging/testing at midpoint
1. Determine which half contains the bug
1. Repeat until you find exact line
...
1. "Instead it does Y"
1. "I think this is because Z"
1. "The code path is: A -> B -> C -> D"
1. "I've verified that..." (list what you tested)
1. "I'm assuming that..." (list assumptions)
...
1. Remove one piece (dependency, function, feature)
1. Test: Does it still reproduce? YES = keep removed. NO = put back.
1. Repeat until bare minimum
1. Bug is now obvious in stripped-down code
...
1. Test that function with expected input - does it produce correct output?
   - YES: Bug is earlier (wrong input)
   - NO: Bug is here
1. Repeat backwards through call stack
1. Find divergence point (where expected vs actual first differ)
...
1. Verify bug is gone
1. Uncomment one piece at a time
1. After each uncomment, test
1. When bug returns, you found the culprit
...
1. Find the code that **consumes** the value (reader/checker/validator)
1. Trace the actual resolved value in both — do they agree?
1. Check every variable in the path construction — where does each come from? What's its actual value at runtime?
...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = <runtime config root>
                  → checks <runtime config root>/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = <managed install root>
                  → writes to <managed install root>/hooks/
...
1. **Binary search** to narrow down where in code
1. **Observability first** to add logging at that point
1. **Rubber duck** to articulate what you're seeing
1. **Minimal reproduction** to isolate just that behavior
1. **Working backwards** to find the root cause
...
1. **You understand why the fix works** - Can explain the mechanism (not "I changed X and it worked")
1. **Related functionality still works** - Regression testing passes
1. **Fix works across environments** - Not just on your machine
1. **Fix is stable** - Works consistently, not "worked once"
...
1. Test each adjacent area manually
1. Run existing tests (unit, integration, e2e)
...
1. **If no answers, switch to reasoning** - Add logging, trace execution
1. **If reasoning reveals gaps, research those specific gaps**
1. **Alternate as needed** - Research reveals what to investigate; reasoning reveals what to research
...
1. Read Current Focus -> know exactly what was happening
1. Read Eliminated -> know what NOT to retry
1. Read Evidence -> know what's been learned
1. Continue from next_action
...
1. Create file with initial state:
   - status: gathering
   - trigger: verbatim $ARGUMENTS
   - Current Focus: next_action = "gather symptoms"
   - Symptoms: empty
1. Proceed to symptom_gathering
   </step>
...
1. Actual behavior -> Update Symptoms.actual
1. Error messages -> Update Symptoms.errors
1. When it started -> Update Symptoms.started
1. Reproduction steps -> Update Symptoms.reproduction
1. Ready check -> Update status to "investigating", proceed to investigation_loop
   </step>
