...
1. Load specific `rules/*.md` files as needed during planning
1. Do NOT load full `AGENTS.md` files (100KB+ context cost)
1. Ensure plans account for project skill patterns and conventions
...
The orchestrator provides user decisions in `<user-decisions>` tags from `/gsd-discuss-phase`.
...
1. **Deferred Ideas (from `## Deferred Ideas`)** — MUST NOT appear in plans

   - If user deferred "search functionality" → NO search tasks allowed
   - If user deferred "dark mode" → NO dark mode tasks allowed

1. **Claude's Discretion (from `## Claude's Discretion`)** — Use your judgment
...
1. **If any D-XX cannot fit** within the plan budget (too many tasks, too complex):
...
1. The orchestrator will present the split to the user for approval
1. After approval, plan each sub-phase within budget
...
1. **Middle tasks: Implement** — Build against the defined contracts
1. **Last task: Wire** — Connect implementations to consumers
...
1. **Account setup** — Does user need to create an account?
1. **Dashboard config** — What must be configured in external UI?
...
**Security (when `security_enforcement` enabled — absent = enabled):** Identify trust boundaries in this phase's scope. Map STRIDE categories to applicable tech stack from RESEARCH.md security domain. For each threat: assign disposition (mitigate if ASVS L1 requires it, accept if low risk, transfer if third-party). Every plan MUST include `<threat-model>` when security_enforcement is enabled.
...
start of execution when `<revision-context>` is provided by the orchestrator.
</revision-mode>
...
- If `<revision-context>` provided by orchestrator: Read `get-shit-done/references/planner-revision.md`
...
1. What does it CREATE? (files, types, APIs others might need)
1. Can it run independently? (no dependencies = Wave 1 candidate)
...
- [ ] `<threat-model>` present with STRIDE register (when `security_enforcement` enabled)
