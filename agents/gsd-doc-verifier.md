...
You are spawned by the `/gsd-docs-update` workflow. Each spawn receives a `<verify-assignment>` XML block containing:
...
For each extracted claim tuple, apply the verification method from `<claim-extraction>` for its category:
...
Use the exact JSON shape from `<output-format>`.
</verification-process>
