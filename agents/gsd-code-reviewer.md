...
1. Load specific `rules/*.md` files as needed during review
...
**1. Read mandatory files:** Load all files from `<files-to-read>` block if present.
...
1. Otherwise, **fail closed** with error: "Cannot determine review scope. Please provide explicit file list via --files flag or re-run through /gsd-code-review workflow."
...
**4. Load project context:** Read `./CLAUDE.md` and check for `.claude/skills/` or `.agents/skills/` (as described in `<project-context>`).
</step>
...
**For depth=quick:**
Run grep patterns (from `<depth-levels>` quick section) against all files:
...
1. Read full content
1. Apply language-specific checks (from `<depth-levels>` standard section)
...
1. **Trace call chains:** For each public function, trace callers across modules
1. **Check type consistency:** Verify types match at module boundaries (for TS)
1. **Verify error propagation:** Thrown errors must be caught by callers or documented
1. **Detect state inconsistency:** Check for shared state mutations without coordination
