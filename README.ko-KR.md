...
[왜 만들었나](#%EC%99%9C-%EB%A7%8C%EB%93%A4%EC%97%88%EB%82%98) · [작동 방식](#%EC%9E%91%EB%8F%99-%EB%B0%A9%EC%8B%9D) · [명령어](#%EB%AA%85%EB%A0%B9%EC%96%B4) · [왜 효과적인가](#%EC%99%9C-%ED%9A%A8%EA%B3%BC%EC%A0%81%EC%9D%B8%EA%B0%80) · [사용자 가이드](docs/ko-KR/USER-GUIDE.md)
...
1. **위치** — 전역 (모든 프로젝트) 또는 로컬 (현재 프로젝트만)
...
1. **리서치** — 도메인 조사를 위해 병렬 에이전트를 생성합니다 (선택사항이지만 권장)
1. **요구사항** — v1, v2, 스코프 밖을 추출합니다
1. **로드맵** — 요구사항에 매핑된 단계를 생성합니다
...
1. **플래너가 읽습니다** — 어떤 결정이 확정됐는지 파악합니다 ("무한 스크롤 결정됨" → 플랜에 스크롤 처리 포함)
...
1. **기획** — XML 구조로 2~3개의 원자적 작업 계획을 생성합니다
1. **검증** — 요구사항 대비 계획을 확인하고, 통과할 때까지 반복합니다
...
1. **계획당 새로운 컨텍스트** — 20만 토큰이 순수하게 구현을 위해, 쌓인 쓰레기 없음
1. **작업당 커밋** — 모든 작업이 고유한 원자적 커밋을 가짐
1. **목표 대비 검증** — 코드베이스가 단계에서 약속한 것을 전달했는지 확인
...
1. **하나씩 안내** — "이메일로 로그인할 수 있나요?" 예/아니오, 또는 뭐가 잘못됐는지 설명
1. **실패 자동 진단** — 근본 원인을 찾기 위해 디버그 에이전트 생성
1. **검증된 수정 계획 생성** — 즉시 재실행 준비 완료
...
| `/gsd-version`      | 설치된 버전, 커밋, 커밋 날짜 표시        |
...
- `/gsd-help` 또는 `/gsd-version`을 실행해 설치 확인
